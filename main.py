            ...
            if src == dest:
                ...
                while len(puis) > 2:
                    ...
                    if self.get_path_with_power(src, dest, power_min) == None:
                        ...
                        power_min = puis[mid - 1]
                if len(
                        puis) >= 2:  # on réalise la dichotomie en fonction de si la puissance donnée permet de réaliser le chemin.
...
# La complexité est de 0((nb_nodes+nb_egdes)*ln(nb_edges))
    ...
    def find_path(self, src, dest,pre_process):
        root=pre_process[0]
        src_chemin=[src]
        dest_chemin=[dest]
        prof = pre_process[3]
    ...
    def min_power_tree(self, src, dest, pre_process):
...
def question_10(route, network,N):  # on cherche à estimer grossièrement le temps de calcul de plusieurs graphes. L'on remarque que ce temps est considérable.
    ...
    with open(route, "r") as file:
        ...
        for i in range(N):
            ...
            g.min_power_tree(dep, arr,pre_process)  # on calcule la puissance minimale pour chaque trajet de routes.x.in
        end = time.time()
        elapsed = end - start
        total = (elapsed / N) * n
...
def dfs(graph, start, profondeur,fathers,visited=None,index=0):
    if visited is None:
    ...
    for neighbor in graph[start]:
            if neighbor[0] not in visited:  # parcourir les voisins non visités
                index+=1
                profondeur[neighbor[0]] += index
                fathers[neighbor[0]] = start
